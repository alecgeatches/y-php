<?php
/**
 * YArray fixture-backed convergence tests.
 *
 * @package Yjs
 */
...
/**
 * Replays deterministic JS YArray operation logs and compares final bytes.
 */
final class YArrayConvergenceTest extends TestCase {
	/**
	 * @return array<int,array{0:array<string,mixed>}>
	 */
	public function convergenceProvider(): array {
		$fixture = $this->fixture( 'yarray-convergence.json' );
		return array_map(
	 ...
	 * @return void
	 */
	public function testYArrayConvergenceMatchesJsBytes( array $case ): void {
		$docs = array();
		for ( $i = 0; $i < $case['users']; $i++ ) {
			$doc           = new Doc( array( 'guid' => 'php-yarray-' . $case['seed'] . '-' . $i ) );
			$doc->clientID = $i + 1;
			$docs[]        = $doc;
		}
		...
		foreach ( $docs as $index => $doc ) {
			self::assertSame(
				$case['json'][ $index ],
				$this->normalizeJsonValue( $doc->getArray( 'array' )->toJSON() ),
	...
	private function applyOperation( array $docs, array $operation ): void {
		$array = $docs[ $operation['user'] ]->getArray( 'array' );
		switch ( $operation['op'] ) {
			case 'insertNumbers':
				$array->insert( $operation['pos'], $operation['content'] );
				break;
			case 'insertArray':
				$array->insert( $operation['pos'], array( new YArray() ) );
				$array->get( $operation['pos'] )->insert( 0, $operation['content'] );
				break;
			case 'insertMap':
				$array->insert( $operation['pos'], array( new YMap() ) );
				$map = $array->get( $operation['pos'] );
				foreach ( $operation['writes'] ?? $operation['entries'] as $entry ) {
					$map->set( (string) $entry[0], $entry[1] );
				}
				break;
			case 'insertNull':
				$array->insert( $operation['pos'], array( null ) );
				break;
			case 'delete':
				$array->delete( $operation['pos'], $operation['len'] );
				break;
			case 'noop':
				break;
			default:
				self::fail( 'Unknown YArray fixture operation: ' . $operation['op'] );
		}
	}
