<?php
/**
 * V1 update codec conformance tests.
 *
 * @package Yjs
 */
...
/**
 * Verifies V1 update-codec byte parity with fixtures captured from real yjs.
 */
final class UpdateCodecV1Test extends TestCase {
	/**
	 * @return array<int,array{0:array<string,mixed>}>
	 */
	public function codecCaseProvider(): array {
		$fixture = $this->fixture( 'update-codecs-v1.json' );
		return array_map(
	 ...
	 * @return void
	 */
	public function testV1CodecPrimitiveMatchesJsBytesAndRoundTrips( array $case ): void {
	...
	/**
	 * @param UpdateEncoderV1 $encoder Encoder.
	 * @param string          $method  Writer method.
	 * @param mixed           $input   Input value.
	 * @return void
	 */
	private function writeUpdateCase( UpdateEncoderV1 $encoder, string $method, $input ): void {
		switch ( $method ) {
			case 'writeLeftID':
	...
	private function readCase( Decoder $decoder, string $method ) {
		...
		$updateDecoder = new UpdateDecoderV1( $decoder );
		switch ( $method ) {
			case 'writeLeftID':
